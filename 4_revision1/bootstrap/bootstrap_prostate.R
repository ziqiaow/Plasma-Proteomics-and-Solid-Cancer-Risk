...
for(prot in sig_proteins) {
  ...
  # Define formula based on cancer type
    ...
    formula_boot <- Surv(incidence_t2, incidence_after_v2)~ x +cigt21+ packyr2 +packyr_missing_indicator  + bmi21 + v2age22 +
                            DIA_DBV2 + UNDIA_DBV2 + ATR_DBV2 + race_center +Factor1 + Factor2 + Factor3 + Factor4+
                            Factor5+Factor6+Factor7+Factor8+Factor9+Factor10+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10
