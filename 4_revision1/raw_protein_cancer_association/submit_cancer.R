...
for(i in 1:dim(prot_norm)[2]){
  ...
  cox.res[[i]] <- coxph(Surv(incidence_t2, incidence_after_v2)~ x + v2age22 +factor(gender) + cigt21 +packyr2 +packyr_missing_indicator  + bmi21  +
...
table(dat$incidence_after_v2)
  ...
  for(i in 1:dim(prot_norm)[2]){
    ...
    dat <- data.frame(x=prot_norm[,i], cancer_incidence)
    cox.res[[i]] <- coxph(Surv(incidence_t2, incidence_after_v2)~ x +cigt21+ packyr2 +packyr_missing_indicator  + bmi21 + v2age22 +
  ...
  names(cox.res) = names(zp) = colnames(prot_norm)
  save(cox.res,zp, file = paste0(in.dir,"cox_fit_fullmodel_colorectal.RData"))
  table(dat$incidence_after_v2)
