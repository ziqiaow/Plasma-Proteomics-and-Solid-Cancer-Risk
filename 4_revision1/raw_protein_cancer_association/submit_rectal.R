...
#minimally adjusted model
  ...
  names(cox.res) = names(zp) = colnames(prot_norm)
  save(cox.res,zp, file = paste0(in.dir,"cox_fit_minimal_colorectal.RData"))
  ...
  covariates$rectal_incidence_v2[which(!is.na(covariates$incidence_t2) & (covariates$rectal_incidence==1))] = 1
  covariates$rectal_incidence_v2[which(is.na(covariates$incidence_t2) & (covariates$prvcancr == 1 | covariates$rectal_incidence==1))] = NA #prevalent cases or missing data at V2
  #because race-center there is no B_F here, so we create a new race-cetner for rectal cancer (simply change it to characters so the factor will reparameterize)
  covariates$race_center_rectal = as.character(covariates$race_center)
  covariates$race_center_rectal[which(covariates$race_center_rectal == "B_F")] = "B_J"
  ...
  for(i in 1:dim(prot_norm)[2]){
    ...
    dat <- data.frame(x=prot_fit[,i], covariates)
    cox.res[[i]] <- coxph(Surv(incidence_t2,rectal_incidence_v2)~ x  +factor(gender) + v2age22 +bmi21+ cigt21 +packyr2 +packyr_missing_indicator+
                            race_center_rectal +PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, dat)
    zp[[i]] <- cox.zph(cox.res[[i]]) #For model diagnostics
    print(i)
  }
  names(cox.res) = names(zp) = colnames(prot_norm)
  save(cox.res,zp, file = paste0(in.dir,"cox_fit_minimal_rectal.RData"))
  ...
  for(i in 1:dim(prot_norm)[2]){
    ...
    dat <- data.frame(x=prot_fit[,i], covariates)
    cox.res[[i]] <- coxph(Surv(incidence_t2,colon_incidence_v2)~ x +factor(gender) + v2age22 +bmi21+ cigt21 +packyr2 +packyr_missing_indicator+
                            race_center +PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, dat)
    zp[[i]] <- cox.zph(cox.res[[i]]) #For model diagnostics
    print(i)
  }
  names(cox.res) = names(zp) = colnames(prot_norm)
  save(cox.res,zp, file = paste0(in.dir,"cox_fit_minimal_colon.RData"))
