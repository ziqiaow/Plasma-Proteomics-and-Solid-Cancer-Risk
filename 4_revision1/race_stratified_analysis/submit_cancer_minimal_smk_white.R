...
#only use white individuals
cancer_incidence = cancer_incidence[which(cancer_incidence$racegrp == "W"),]
...
if(cancer =="prostate"){
  ...
  save(cox.res,zp, file = paste0(in.dir,"cox_fit_minimal_",cancer,"_smk_white.RData"))
  ...
  names(cox.res) = names(zp) = colnames(prot_norm)
  save(cox.res,zp, file = paste0(in.dir,"cox_fit_minimal_colorectal_smk_white.RData"))
  ...
  names(cox.res) = names(zp) = colnames(prot_norm)
  save(cox.res,zp, file = paste0(in.dir,"cox_fit_minimal_rectal_smk_white.RData"))
  ...
  names(cox.res) = names(zp) = colnames(prot_norm)
  save(cox.res,zp, file = paste0(in.dir,"cox_fit_minimal_colon_smk_white.RData"))
  ...
  for(i in 1:dim(prot_norm)[2]){
    ...
    dat <- data.frame(x=prot_norm[,i], cancer_incidence)
    cox.res[[i]] <- coxph(Surv(incidence_t2, incidence_after_v2)~ x  +factor(gender) +bmi21+ cigt21 +packyr2 + v2age22 +
                            as.factor(center) +PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10, dat)
    zp[[i]] <- cox.zph(cox.res[[i]]) #For model diagnostics
    print(i)
  }
